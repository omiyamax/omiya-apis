...
function news(){
	return Promise(async(resolve, reject) => {
		axios.get('http://sinhala.adaderana.lk/sinhala-hot-news.php')
		.then(({ data }) => {
			const $ = cheerio.load(data)
			const hasil = [];
			$('div.news-story div.story-text h2').find('a').each(function(i, elem) {
				const url = "http://sinhala.adaderana.lk/" + elem.attribs.href
				const text = elem.children[0].data
			$('div.news-story div.story-text div.thumb-image').find('img').each(function(i, elem) {
					const img = elem.attribs.src;
				const result = {
					message: 'Hiruwa',
					indo : {
						text: text,
...
module.exports = { ttdownloader, youtube, twitter, fbdown, igstalk, igstory, igdl, pinterest, linkwa, playstore, wattpad, chara, lirik, sholat, news }
